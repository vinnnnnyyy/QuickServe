...
const sampleTables = [
  {
    ...
    qrCode: 'QR_TABLE_001',
    sessions: [],
    description: 'Cozy window-side table perfect for small meetings or individual dining.',
    notes: 'Near the main entrance, good for quick service.'
  },
  {
    id: 2,
    number: 2,
    ...
    description: 'Central family table with comfortable seating.',
    notes: 'Popular table, often booked in advance.'
  },
  {
    id: 5,
    number: 5,
    ...
    description: 'Outdoor patio table with garden view.',
    notes: 'Weather dependent, has umbrella for shade.'
  }
];
...
const loadTable = () => {
  ...
  const table = sampleTables.find(table => table.id == props.id);
  ...
  originalTable.value = { ...table };
  
  // Map location from display name to value
  const locationValue = locationOptions.value.find(opt => opt.label === table.location)?.value || 'indoor';
...
const cancelNewLocation = () => {
  showNewLocation.value = false;
  newLocation.value = '';
  form.value.location = originalTable.value ? 
    locationOptions.value.find(opt => opt.label === originalTable.value.location)?.value || 'indoor' : 
    'indoor';
};
...
const cancelNewCapacity = () => {
  showNewCapacity.value = false;
  newCapacity.value = '';
  form.value.capacity = originalTable.value ? originalTable.value.capacity.toString() : '4';
};
...
const submitForm = () => {
  ...
  const updatedTable = {
    ...
    capacity: capacity,
    occupied: originalTable.value?.occupied || 0,
    status: originalTable.value?.status || 'available',
    statusColor: originalTable.value?.statusColor || 'bg-green-200 dark:bg-green-800 border-green-200 dark:border-green-800',
    statusText: originalTable.value?.statusText || 'Available',
    statusDot: originalTable.value?.statusDot || 'bg-green-500',
    qrCode: generateQRCode(tableNumber),
    sessions: originalTable.value?.sessions || [],
  ...
  alert(`Table ${form.value.number} has been updated successfully!`);
...
const saveDraft = () => {
  localStorage.setItem('tableEditDraft', JSON.stringify(form.value));
...
const deleteTable = () => {
  if (confirm(`Are you sure you want to delete Table ${originalTable.value?.number}? This action cannot be undone.`)) {
    console.log('Deleting table:', props.id);
    alert(`Table ${originalTable.value?.number} has been deleted successfully!`);
    router.get('/admin/tables');
  }
};
...
<template>
  <AdminLayout 
    ...
    page-subtitle="Update your table details and configuration"
  >
    <!-- Loading State -->
    <div v-if="loading" class="flex items-center justify-center min-h-64">
      <div class="text-center">
        ...
        <p class="text-gray-600 dark:text-gray-400">Loading table...</p>
      </div>
    </div>
        ...
        <form @submit.prevent="submitForm" class="space-y-12">
          ...
          <CardWrapper rounded="xl" padding="lg" shadow="hover" hover>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <!-- Table Status & Info -->
              <div class="space-y-4">
                <label class="block text-sm font-semibold text-gray-900 dark:text-white">Current Status</label>
                  ...
                  <div class="text-center">
                    ...
                    <div class="space-y-3">
                      <div class="flex items-center justify-center gap-2">
                        <span class="text-2xl font-bold text-gray-900 dark:text-white">{{ form.number || originalTable?.number }}</span>
                        <span 
                          v-if="form.location || originalTable?.location" 
                          class="text-xs px-2 py-1 rounded-full"
                          :class="form.location ? getLocationConfig(form.location).color : originalTable?.locationColor"
                        >
                          {{ form.location ? getLocationConfig(form.location).label : originalTable?.location }}
                        </span>
                      </div>
                      ...
                      <div class="text-sm space-y-1">
                        <div class="flex items-center justify-between">
                          ...
                          <span class="font-medium">{{ form.capacity || originalTable?.capacity }} {{ (form.capacity || originalTable?.capacity) === '1' ? 'seat' : 'seats' }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                          <span class="text-gray-600 dark:text-gray-400">Occupied:</span>
                          <span class="font-medium">{{ originalTable?.occupied || 0 }}/{{ form.capacity || originalTable?.capacity }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                          <span class="text-gray-600 dark:text-gray-400">Status:</span>
                          <span class="flex items-center gap-1">
                            <span :class="originalTable?.statusDot" class="w-2 h-2 rounded-full"></span>
                            <span class="text-sm font-medium">{{ originalTable?.statusText || 'Available' }}</span>
                          </span>
                        </div>
                        <div class="flex items-center justify-between">
                          <span class="text-gray-600 dark:text-gray-400">QR Code:</span>
                          <span class="text-xs font-mono">{{ generateQRCode(form.number || originalTable?.number) }}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <!-- Active Sessions -->
                <div v-if="originalTable?.sessions?.length > 0" class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-200 dark:border-blue-800">
                  ...
                  <div class="space-y-2">
                    <div v-for="session in originalTable.sessions" :key="session.id" class="text-xs">
                      <div class="flex items-center justify-between">
                        <span class="font-mono text-blue-800 dark:text-blue-200">{{ session.id }}</span>
                        <span class="px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-800 text-blue-800 dark:text-blue-200">{{ session.status }}</span>
                      </div>
                    </div>
                  </div>
                </div>
