...
const sampleOrders = [
  {
    id: 'A001',
    sessionId: '#1247',
    tableType: 'Table 5 (Patio, 4 seats)',
    orderType: 'Dine In',
    ...
    items: {
      ...
      details: [
        { id: 1, name: 'Iced Brown Sugar Oatmilk Shaken Espresso', price: 5.45, quantity: 1, category: 'Cold Drinks' },
        { id: 5, name: 'Croissant', price: 3.50, quantity: 1, category: 'Pastries' },
        { id: 6, name: 'Blueberry Muffin', price: 4.25, quantity: 1, category: 'Pastries' }
      ]
    },
    total: 18.90,
    subtotal: 16.88,
    ...
    specialInstructions: 'Extra hot, oat milk instead of regular',
  ...
  },
  {
    ...
    items: {
      ...
      details: [
        { id: 3, name: 'Classic Americano', price: 3.85, quantity: 1, category: 'Hot Drinks' }
      ]
    },
    total: 4.95,
    subtotal: 4.42,
...
const loadOrder = () => {
  ...
  const order = sampleOrders.find(order => order.id === props.id);
  ...
  // Map table from display name to value
  const tableValue = tableOptions.value.find(opt => opt.label === order.tableType)?.value || 'table_1';
  const orderTypeValue = orderTypeOptions.find(opt => opt.label === order.orderType)?.value || 'dine_in';
  const deviceTypeValue = deviceTypeOptions.find(opt => opt.label === order.deviceInfo.type)?.value || 'mobile';
  ...
  // Load order items
  orderItems.value = [...(order.items.details || [])];
...
const cancelNewTable = () => {
  showNewTable.value = false;
  newCustomTable.value = '';
  form.value.tableType = originalOrder.value ? 
    tableOptions.value.find(opt => opt.label === originalOrder.value.tableType)?.value || 'table_1' : 
    'table_1';
};
...
const submitForm = () => {
  ...
  const updatedOrder = {
    ...
    status: originalOrder.value?.status || {
      text: 'Pending',
      color: 'orange'
    },
    specialInstructions: form.value.specialInstructions,
    notes: form.value.notes,
    time: originalOrder.value?.time || 'Just now',
    updatedAt: new Date().toISOString()
  };

  console.log('Updated order:', updatedOrder);
  alert(`Order ${props.id} has been updated successfully!`);
...
const saveDraft = () => {
  ...
  localStorage.setItem('orderEditDraft', JSON.stringify(draft));
...
const deleteOrder = () => {
  if (confirm(`Are you sure you want to delete Order ${props.id}? This action cannot be undone.`)) {
    console.log('Deleting order:', props.id);
    alert(`Order ${props.id} has been deleted successfully!`);
    router.get('/admin/orders');
  }
};
...
<template>
  <AdminLayout 
    ...
    page-subtitle="Update order details and items"
  >
    <!-- Loading State -->
      ...
      <div class="max-w-7xl mx-auto">
        <form @submit.prevent="submitForm" class="space-y-12">
          ...
          <CardWrapper rounded="xl" padding="lg" shadow="hover" hover>
            ...
            <div class="mb-8">
              <div class="bg-white dark:bg-gray-900/50 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
                <label class="block text-sm font-semibold text-gray-900 dark:text-white mb-4">Current Order Status</label>
                
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div class="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <p class="text-sm text-gray-600 dark:text-gray-400">Order ID</p>
                    <p class="font-bold text-gray-900 dark:text-white">{{ originalOrder?.id }}</p>
                  </div>
                  <div class="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <p class="text-sm text-gray-600 dark:text-gray-400">Status</p>
                    <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium"
                          :class="{
                            'bg-orange-100 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400': originalOrder?.status.color === 'orange',
                            'bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400': originalOrder?.status.color === 'blue',
                            'bg-yellow-100 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400': originalOrder?.status.color === 'yellow',
                            'bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400': originalOrder?.status.color === 'purple',
                            'bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400': originalOrder?.status.color === 'green'
                          }"
                    >
                      {{ originalOrder?.status.text }}
                    </span>
                  </div>
                  <div class="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <p class="text-sm text-gray-600 dark:text-gray-400">Payment</p>
                    <span class="inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium"
                          :class="{
                            'bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400': originalOrder?.payment.color === 'green',
                            'bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400': originalOrder?.payment.color === 'red'
                          }"
                    >
                  ...
                  <div class="text-center p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    ...
                    <p class="font-medium text-gray-900 dark:text-white">{{ originalOrder?.time }}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <CardWrapper rounded="xl" padding="lg" shadow="hover" hover>
            <FormSection 
              title="Order Items"
              subtitle="Manage items in this order"
              icon="shopping_cart"
            />
            
            <!-- Selected Items -->
            <div v-if="orderItems.length > 0" class="mb-8">
              <div class="bg-white dark:bg-gray-900/50 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
                <div class="flex items-center justify-between mb-4">
                  <label class="block text-sm font-semibold text-gray-900 dark:text-white">Order Items</label>
                  <button 
                    type="button"
                    @click="clearCart"
            ...
            <div class="mb-8">
              <div class="bg-white dark:bg-gray-900/50 rounded-xl p-6 border border-gray-200 dark:border-gray-700">
                <label class="block text-sm font-semibold text-gray-900 dark:text-white mb-6">Add More Items</label>
            ...
            <!-- Empty State -->
            <div v-if="orderItems.length === 0" class="text-center py-12 bg-gray-50 dark:bg-gray-900/20 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-700">
              <div class="w-16 h-16 mx-auto mb-4 rounded-full bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
                <span class="material-symbols-outlined text-gray-400 text-2xl">shopping_cart</span>
              </div>
              <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">No items in order</h3>
              <p class="text-gray-600 dark:text-gray-400 mb-4">Add items from the menu above to continue</p>
            </div>
          </CardWrapper>
          ...
          <CardWrapper rounded="xl" padding="lg" shadow="sm">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-6">
              ...
              <!-- Right side - Action Buttons with Order Summary -->
